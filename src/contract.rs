...
fn try_roll_dice(deps: DepsMut, env: Env, info: MessageInfo) -> Result<Response, ContractError> {
    let mut state = config(deps.storage).load()?;
    ...
    match state.state {
        ...
        ContractState::Got2 => {
            ...
            dice_roll = ((rng.next_u32() % 6) + 1) as u8;
        ...
        // Had a player already won the game?
        ContractState::Done => {
            return Err(ContractError::GameIsAlreadyOver);
        }
    }
...
fn query_who_won(deps: Deps, env: Env) -> StdResult<WinnerResponse> {
    ...
    if current_height <= winner_height {
        return Err(StdError::generic_err(
            "Quering who won is not allowed until after the winner has been finalized.",
        ));
    }
    ...
    let winner = if let Some(winner) = &state.winner {
        ...
        return Err(StdError::generic_err("Winner not fount."));
    };
